...
export function PhotoLightbox({ initialIndex, onClose }: PhotoLightboxProps) {
  ...
  return createPortal(
    <div aria-label="Photo viewer" aria-modal="true" className="fixed inset-0 z-50 flex items-center justify-center bg-black/92 p-8 backdrop-blur-[10px] max-[700px]:px-14 max-[700px]:py-4" onClick={onClose} role="dialog">
      <button aria-label="Close photo viewer" className="modal-control-focus absolute top-4 right-6 z-1 border-0 bg-transparent text-3xl text-white/50 transition-colors hover:text-white" onClick={onClose}><Icon name="close" /></button>
      <button aria-label="Previous photo" className="modal-control-focus absolute top-1/2 left-4 z-1 flex size-16 -translate-y-1/2 items-center justify-center text-5xl text-white/70 transition-colors hover:text-white max-[700px]:left-1 max-[700px]:size-12 max-[700px]:text-4xl" onClick={(event) => { event.stopPropagation(); previous() }}><Icon name="chevron_left" /></button>
