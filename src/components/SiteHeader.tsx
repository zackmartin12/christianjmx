...
export function SiteHeader({ activeTab, menuOpen, onSelectContact, onSelectTab, onToggleMenu, tabs }: SiteHeaderProps) {
  return <header className="sticky top-0 z-20 border-b border-[rgb(210_215_218_/_9%)] [background:rgb(17_19_20_/_95%)] backdrop-blur-[12px]">
    <div className="flex items-center justify-between px-16 min-[701px]:max-[1000px]:px-12 max-[700px]:px-8">
      ...
      <nav aria-label="Portfolio sections" className="flex self-stretch max-[700px]:hidden">
        ...
        <button className={navigationButtonClass(false)} onClick={onSelectContact}>Contact</button>
      </nav>
      <button aria-controls="mobile-menu" aria-expanded={menuOpen} className="hidden border-0 bg-transparent py-4 text-[1.3rem] text-white/70 transition-colors hover:text-white max-[700px]:block" onClick={onToggleMenu}>
        <Icon name={menuOpen ? 'close' : 'menu'} /><span className="sr-only">Toggle menu</span>
      </button>
    </div>
    {menuOpen && <nav aria-label="Portfolio sections" className="hidden border-t border-[rgb(210_215_218_/_9%)] max-[700px]:flex max-[700px]:flex-col" id="mobile-menu">
      ...
      <button className="border-0 bg-transparent px-8 py-4 text-left font-condensed text-xs tracking-[.15em] text-[#6a6f73] uppercase" onClick={onSelectContact}>Contact</button>
    </nav>}
  </header>
}
